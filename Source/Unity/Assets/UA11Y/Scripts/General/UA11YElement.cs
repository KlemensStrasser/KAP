...
[AddComponentMenu("UA11Y/UA11YElement")]
public class UA11YElement : MonoBehaviour
{
    ...
    /// This and the following functions are partially based on: https://answers.unity.com/questions/292031/how-to-display-a-rectangle-around-a-player.html
    ...
    protected static Rect ScreenRectForGameObject(GameObject gObject)
    {
        ...
        if (gObject.GetComponent<Renderer>())
        {
            ...
            rect = ScreenRectForRenderer(renderer);
        ...
        else
        {
            ...
            if (renderers.Count() > 0)
            {
                ...
                foreach (Renderer nextRenderer in renderers)
                {
                    Rect nextRect = ScreenRectForRenderer(nextRenderer);
                    ...
                    {
                        float minX = Mathf.Min(rect.x, nextRect.x);
                        float minY = Mathf.Min(rect.y, nextRect.y);
                        rect.width = Mathf.Max(rect.x + rect.width, nextRect.x + nextRect.width) - minX;
                        rect.height = Mathf.Max(rect.y + rect.height, nextRect.y + nextRect.height) - minY;
                        rect.x = minX;
                        rect.y = minY;
                    }
                }
        ...
        if (!hasRenderer)
        {
            ...
            {
                rect = ScreenRectForRectTransform(rectTransform);
            }
        }
    ...
    protected static Rect ScreenRectForRenderer(Renderer renderer)
    {
        Rect rect;
